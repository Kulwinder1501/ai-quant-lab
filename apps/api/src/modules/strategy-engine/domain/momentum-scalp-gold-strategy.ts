...
/**
 * The same rule set as `momentum-scalp-index` (EMA separation confirmed by Supertrend direction
 * and a bounded RSI band, ATR-based stop/target), registered under its own key for XAU_USD.
 *
 * Why a fork rather than reusing `momentum-scalp-index` directly: that strategy is registered but
 * `executableSides: []` -- disabled on both sides in `strategy-registry.ts`, with a
 * `terminalResearchAcknowledgement` recording a measured verdict (-Rs 33,449 net across NIFTY50
 * and BANKNIFTY, closed by a research horizon sweep that found NO_VIABLE_HORIZON). Enabling that
 * registration globally to trade gold would also silently resurrect it for AutoBot-Classic on the
 * Indian indices it was deliberately turned off for -- a strategy key is shared by every bot that
 * lists it. This file keeps the same, already instrument-agnostic entry/stop/target math (it has
 * no VWAP/session dependency, unlike `momentum-scalp`'s `NSE_SESSION` reset) but gives gold its
 * own key, so its live record accumulates independently and never re-arms the Indian-index verdict.
 *
 * Registered with no `operationalDisposition` and no `executableSides` restriction: it has never
 * traded, on any instrument, under this key, so there is no evidence either way yet -- the same
 * position `momentum-scalp` started from on AutoBot-Scalp1m (see bot-sandboxes.ts).
 */
...
export const defaultMomentumScalpGoldStrategyConfiguration: MomentumScalpGoldStrategyConfiguration = {
...
export const momentumScalpGoldStrategyRegistration = {
  strategyKey: "momentum-scalp-gold",
  name: "Momentum Scalp (Gold)",
  description: "Fast EMA separation confirmed by Supertrend direction and a bounded RSI momentum band, for XAU_USD.",
  version: momentumScalpGoldStrategyVersion,
  configuration: { ...defaultMomentumScalpGoldStrategyConfiguration } as Record<string, unknown>,
};

function requiredNumber(raw: Record<string, unknown>, key: string, min = -Infinity, max = Infinity): number {
  const value = raw[key];
  if (typeof value !== "number" || !Number.isFinite(value) || value < min || value > max) {
    throw new Error(`Momentum scalp gold configuration requires a valid numeric ${key} between ${min} and ${max}.`);
  }
  return value;
}
...
function requiredString(raw: Record<string, unknown>, key: string): string {
  ...
  if (typeof value !== "string" || value.trim().length === 0) {
    throw new Error(`Momentum scalp gold configuration requires a valid string ${key}.`);
  }
  return value.trim();
}
...
function requiredBoolean(raw: Record<string, unknown>, key: string): boolean {
  if (typeof raw[key] !== "boolean") {
    throw new Error(`Momentum scalp gold configuration requires a boolean ${key}.`);
  }
...
function requiredIndicatorParameters(raw: Record<string, unknown>): Record<string, Record<string, number | string | boolean>> {
  ...
  if (!candidate || typeof candidate !== "object" || Array.isArray(candidate)) {
    throw new Error("Momentum scalp gold configuration requires indicator parameter sets.");
  }
  ...
  for (const code of ["EMA_FAST", "EMA_SLOW", "RSI", "SUPERTREND", "ATR"]) {
    ...
    if (!parameters || typeof parameters !== "object" || Array.isArray(parameters) || Object.keys(parameters).length === 0) {
      throw new Error(`Momentum scalp gold configuration requires parameters for ${code}.`);
    }
    ...
    for (const [key, value] of Object.entries(parameters as Record<string, unknown>)) {
      if (typeof value !== "number" && typeof value !== "string" && typeof value !== "boolean") {
        throw new Error(`Momentum scalp gold configuration has an unsupported parameter for ${code}.`);
      }
      typed[key] = value;
    }
    result[code] = typed;
...
export function parseMomentumScalpGoldStrategyConfiguration(raw: Record<string, unknown>): MomentumScalpGoldStrategyConfiguration {
  const configuration: MomentumScalpGoldStrategyConfiguration = {
  ...
  if (configuration.rsiLongMax <= configuration.rsiLongMin) {
    throw new Error("Momentum scalp gold configuration requires rsiLongMax to be greater than rsiLongMin.");
  }
  if (configuration.rsiShortMax <= configuration.rsiShortMin) {
    throw new Error("Momentum scalp gold configuration requires rsiShortMax to be greater than rsiShortMin.");
  }
...
function resolveIndicators(context: StrategyMarketContext, configuration: MomentumScalpGoldStrategyConfiguration): ResolvedIndicators | null {
...
function rsiMomentumScore(rsiValue: number, side: TradeSide, configuration: MomentumScalpGoldStrategyConfiguration): number {
...
function supertrendBandScore(close: number, supertrendValue: number, atrValue: number): number {
...
function confidenceFor(
  ...
  configuration: MomentumScalpGoldStrategyConfiguration,
  ...
  const supertrendScore = supertrendBandScore(context.candle.close, indicators.supertrendValue, indicators.atrValue);
...
function buildProposal(
  ...
  const evidenceItems: TradeIdeaEvidence[] = [
    ...
    },
    {
      ...
      sourceReference: `momentum-scalp-gold:v${momentumScalpGoldStrategyVersion}`,
      label: `Momentum Scalp Gold v${momentumScalpGoldStrategyVersion} rule set passed`,
      contribution: null,
      details: {
        configuration,
  ...
  return {
    ...
    confidence: scores.confidence,
    reasoning: [
      `${labelSide} Momentum Scalp Gold entry on ${context.candle.timeframe}.`,
    ...
    evidence: {
      strategy: "momentum-scalp-gold",
      strategyVersion: momentumScalpGoldStrategyVersion,
      sourceCandleId: context.candle.id,
...
export class MomentumScalpGoldStrategy {
  evaluate(context: StrategyMarketContext, rawConfiguration: Record<string, unknown>): ProposedTradeIdea[] {
    const configuration = parseMomentumScalpGoldStrategyConfiguration(rawConfiguration);
