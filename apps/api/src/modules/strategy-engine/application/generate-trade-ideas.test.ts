...
describe("GenerateTradeIdeas higher-timeframe attachment", () => {
  ...
  it("drops LONG proposals when the attached 15m candle carries a BEARISH_SWEEP", async () => {
    const htfClose = new Date("2026-07-25T05:00:00Z");
    const baseCtx = bullishScalpContext("candle-1m");
    const htf15m = contextAt("15m", htfClose, 102);
    htf15m.indicators = [
      {
        code: "LIQUIDITY_SWEEP",
        algorithmVersion: "smc-v2",
        parameters: {},
        values: { type: "BEARISH_SWEEP", level: 105 },
      } as never,
    ];

    const { generator } = harness({
      findLatestCompleted: async () => baseCtx,
      findCompletedBefore: async (input) => (input.timeframe === "15m" ? htf15m : null),
    });

    const result = await generator.execute({ instrumentId: "instrument-1", timeframe: "1m" });
    const scalpResult = result.find((r) => r.strategyKey === "momentum-scalp");
    expect(scalpResult?.candidatesGenerated).toBe(0);
    expect(scalpResult?.skippedReason).toBe("RULES_NOT_MET");
  });

  it("admits LONG proposals and attaches evidence when the attached 15m candle carries a BULLISH_SWEEP", async () => {
    const saved: SaveTradeIdeaProposalInput[] = [];
    const htfClose = new Date("2026-07-25T05:00:00Z");
    const baseCtx = bullishScalpContext("candle-1m");
    const htf15m = contextAt("15m", htfClose, 102);
    htf15m.indicators = [
      {
        ...
        algorithmVersion: "smc-v2",
        parameters: {},
        values: { type: "BULLISH_SWEEP", level: 99 },
      } as never,
    ];

    const versions = passthroughStrategyVersions();
    const ideas = recordingIdeas(saved);
    const contexts: StrategyMarketContextRepository = {
      findLatestCompleted: async () => baseCtx,
      findCompletedBefore: async (input) => (input.timeframe === "15m" ? htf15m : null),
      listCompletedContexts: async () => [],
    ...
    const generator = new GenerateTradeIdeas(versions, contexts, ideas);
    const result = await generator.execute({ instrumentId: "instrument-1", timeframe: "1m" });
    const scalpResult = result.find((r) => r.strategyKey === "momentum-scalp");
    expect(scalpResult?.candidatesGenerated).toBe(1);
    expect(saved[0].side).toBe("LONG");
    expect(saved[0].evidence.htfLiquidityBias).toMatchObject({
      timeframe: "15m",
      bias: "BULLISH",
    });
  });
