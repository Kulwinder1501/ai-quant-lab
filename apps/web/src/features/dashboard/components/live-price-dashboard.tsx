...
import { MarketWatchSelect } from "./market-watch";
...
export function LivePriceDashboard() {
  ...
  /**
   * Fixed at the daily series now that the 15m/1h/1d switcher has been removed from the header.
   *
   * Kept as a named constant rather than inlined into the stream URL because `/stream/live-agent`
   * takes it as a query parameter and defaults to `1d` server-side: making the value explicit here
   * keeps the client and that default from drifting apart silently.
   */
  const timeframe = "1d";
      ...
      */}
      <div className="flex flex-col flex-1 mt-2 font-sans">
        <Reveal className="flex flex-col w-full">
          {/* `relative z-30` is what lets the watch-list dropdown escape this panel.
              GlassPanel applies `backdrop-blur`, and a backdrop-filter creates a stacking context,
              so the panel's own `z-50` was resolved *inside* this header and the cards further down
              the page -- later siblings at `z-index: auto` -- still painted over it. Giving the
              header a z-index of its own lifts the whole subtree above them. */}
          <GlassPanel className="relative z-30 mb-3 p-4 border-white/10 bg-slate-900/60 shadow-lg shrink-0">
            <div className="flex flex-wrap items-center justify-between gap-4 px-2">
              ...
              {/* The watch list itself is the instrument selector: same rows, same prices, same
                  AI stance as the old side panel, so switching symbol and scanning the market are
                  one control rather than two views of the same feed. */}
              <div className="flex items-center gap-3">
                <MarketWatchSelect selectedSymbol={selectedSymbol} onSelect={selectSymbol} />
              </div>
            </div>
          </GlassPanel>
          ...
          {state !== "ready" || !data ? (
            ...
            <div className="flex flex-col gap-4 pb-6">
              ...
              {/* Card 1: full-width chart, heatmap stacked beneath it.
                  The chart no longer shares the row with the watch list (now the header selector)
                  or the option-chain metrics (removed), so it takes the whole width. */}
